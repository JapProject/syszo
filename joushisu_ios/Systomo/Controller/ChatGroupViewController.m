...
@interface ChatGroupViewController ()<UITableViewDataSource, UITableViewDelegate, UITextViewDelegate,AppDelegateViewControllerDelegate>
...
- (void)passValue:(NSDictionary *)dic
{
    ...
    NSString *group_id = [dic valueForKey:@"group_id"];
    if ([self.group_id isEqualToString:group_id]) {
        /// 判断推送的是否是当前消息
        
        /// 改成网络请求刷新列表
        [self.dataArray removeAllObjects];
        [self PostMessageWithGroup_id:self.group_id page:@"1"];
        [self.tableView reloadData];
    }
    
    
}
- (void)setGroup_id:(NSString *)group_id
{
...
- (void)createSend
{
    ...
    //    self.send.borderStyle = UITextBorderStyleRoundedRect;
...
- (void)chatAction:(UIButton *)sender
{
    ...
    if (![self.postEmall isEqualToString:@""]) {
        ...
        NSDictionary *parameters = @{@"user_id":user.user_id,@"group_id":self.group_id,@"info_content":self.postEmall,@"info_img":@""};
...
- (void)uploadingImage:(NSDictionary *)info
{
    ...
    __weak ChatGroupViewController *systomoGropAddVC = self;
    AFHTTPRequestOperationManager *manager = [AFHTTPRequestOperationManager manager];
    manager.responseSerializer.acceptableContentTypes = [NSSet setWithObject:@"text/html"];
...
- (void)sendImage
{
    ...
    UserModel *user = [UserModel unpackUserInfo];
    //传入的参数 单聊 group_id 为空
    NSDictionary *parameters = @{@"user_id":user.user_id,@"group_id":self.group_id,@"info_content":@"",@"info_img":self.picUrlString};
    AFHTTPRequestOperationManager *manager = [AFHTTPRequestOperationManager manager];
    manager.responseSerializer.acceptableContentTypes = [NSSet setWithObject:@"text/html"];
    [manager POST:str parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
        NSString *result = [responseObject objectForKey:@"result"];
...
- (void) keyboardWasShown:(NSNotification *) notif
{
    ...
    [UIView animateWithDuration:duration animations:^{
        [self.sendView setFrame:CGRectMake(0, CON_HEIGHT - 40 - 64 - keyboardSize.height, CON_WIDTH, 44)];
    }];
}
/// 键盘收回
...
//}
/// 网络请求列表
- (void)PostMessageWithGroup_id:(NSString *)group_id page:(NSString *)page
{
    NSString *str = [NSString stringWithFormat:@"%@%@", TiTleUrl,ChatInfoUrl];
    //    //传入的参数 单聊 group_id 为空
    UserModel *user = [UserModel unpackUserInfo];
    //传入的参数
    NSDictionary *parameters = @{@"user_id":user.user_id,@"group_id":group_id,@"p_num":page, @"p_size":@"20"};
    __weak ChatGroupViewController *chieGroupVC = self;
    AFHTTPRequestOperationManager *manager = [AFHTTPRequestOperationManager manager];
    manager.responseSerializer.acceptableContentTypes = [NSSet setWithObject:@"text/html"];
    [manager POST:str parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
        NSString *result = [responseObject objectForKey:@"result"];
        ...
        for (NSDictionary *dic in tempArr) {
            ChatModel *model = [[ChatModel alloc] init];
            [model setValuesForKeysWithDictionary:dic];
            [chieGroupVC.dataArray addObject:model];
        }
...
- (void)headerRereshing
{
    ...
    [self PostMessageWithGroup_id:self.group_id page:pageStr];
    self.page ++;
