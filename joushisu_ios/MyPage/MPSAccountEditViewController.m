//
//  MPSAccountEditViewController.m
//  joushisu_ios
//
...
@interface MPSAccountEditViewController ()<UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate, UIAlertViewDelegate, NADViewDelegate>
...
@implementation MPSAccountEditViewController
...
- (void)viewDidLoad {
    ...
    self.titleArray = [NSMutableArray arrayWithObjects:@"現在のパスワード(6～16文字以内)", @"新しいパスワード（6～16文字以内）", nil];
    self.titleArray1 = [NSMutableArray arrayWithObjects:@"パスワード", @"パスワード(確認用)", nil];
...
{
    NSLog(@"旧密码==%@,\n新密码==%@\n,新密码重复==%@", self.oldPassWord, self.nePassWord, self.nePassWord2);
    if ([self.nePassWord isEqualToString:self.nePassWord2] && self.nePassWord2.length >= 6 && self.nePassWord2.length <= 16) {
        [self creatDataWithUser_id:self.user.user_id user_pwd:self.nePassWord2 old_pwd:self.oldPassWord];
    } else {
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"パスワードを再度ご確認ください" message:nil delegate:self cancelButtonTitle:@"キャンセル" otherButtonTitles:nil];
        [alertView show];
        return;
    }
}

- (void)creatDataWithUser_id:(NSString *)user_id user_pwd:(NSString *)user_pwd old_pwd:(NSString *)old_pwd
{
    NSString *str = [NSString stringWithFormat:@"%@%@", TiTleUrl,MyPagePWSEdit];
    //传入的参数
    NSDictionary *parameters = @{@"user_id":user_id,@"user_pwd":user_pwd, @"old_pwd":old_pwd};
    //    NSLog(@"网络请求里面的type:%@, page:%@", type, page);
    __weak MPSAccountEditViewController *chieVC = self;
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (alertView.tag == 20499) {
        ...
        NSLog(@"修改密码成功, 返回上级画面,本地修改密码");
        UserModel *user = [UserModel unpackUserInfo];
        user.user_password = self.nePassWord;
...
//            [[NSNotificationCenter defaultCenter] removeObserver:self name:UIKeyboardWillHideNotification object:nil];
            ...
            break;
        case 23840 + (1 * 2):/// 第二个section的第一个输入框
        {
            // 注册两个通知来监听键盘高度变化
            // 键盘弹出
            ...
            break;
        case 23840 + (2 * 2):/// 第二个section的第二个输入框
        {
            // 注册两个通知来监听键盘高度变化
            // 键盘弹出
            [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(keyboardWasShown:) name:UIKeyboardWillShowNotification object:nil];
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    switch (textField.tag) {
        case 23840 + (1 * 1):/// 第一个section的输入框
        {
            self.oldPassWord = [NSString stringWithFormat:@"%@%@", textField.text, string];
        }
            break;
        case 23840 + (1 * 2):/// 第二个section的第一个输入框
        {
            self.nePassWord = [NSString stringWithFormat:@"%@%@", textField.text, string];
        }
            break;
        case 23840 + (2 * 2):/// 第二个section的第二个输入框
        {
            self.nePassWord2 = [NSString stringWithFormat:@"%@%@", textField.text, string];
        }
            break;
        default:
            break;
